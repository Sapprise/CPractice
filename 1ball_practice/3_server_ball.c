...
void init_time() {
    int id = length[59];
    itime[id].it_value.tv_sec = 0;
    itime[id].it_value.tv_usec = 20000;

    itime[id].it_interval.tv_sec = 0;
    itime[id].it_interval.tv_usec = 0;
    setitimer(ITIMER_REAL, &itime[id], NULL);
}
...
void ti() {
    int id = length[59];
    printf("%ld\n", itime[id].it_value.tv_usec);
    if (itime[id].it_value.tv_usec > 60000) {
        return ;
    }
    one_ti();
    itime[id].it_value.tv_usec += 20000;
...
void *all_people(void *str) {
    ...
    while (1) {
        usleep(10000);
        read(st->st_sock[id], my_signal, sizeof(signal));
        //printf("第%d玩家操作:%s\n", id, signal );
        if (my_signal[0] == 'w') {
            dir1 = -1, dir2 = 0; 
        }else if (my_signal[0] == 's') {
            dir1 = 1, dir2 =0; 
        }else if (my_signal[0] == 'a') {
            dir1 = 0, dir2 = -1; 
        }else if (my_signal[0] == 'd') {
            dir1 = 0, dir2 = 1; 
        }else if (my_signal[0] == ' ' && abs(le - length[1]) <= 1 && abs(pos - length[2]) <= 1) {
            //ti();
            signal(SIGALRM, ti);
            my_signal[0] = 'a';
            init_time();
